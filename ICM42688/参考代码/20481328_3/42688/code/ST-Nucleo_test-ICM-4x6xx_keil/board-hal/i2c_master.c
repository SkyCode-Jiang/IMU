...
{
    //return i2c_write(IIC_ADD, offset, &data,1);
    return i2c_master_write_register(ICM_I2C_ADDR, offset, 1, &data);
}

int i2c_read_1B(unsigned short offset, unsigned char * data)
{
    //return i2c_read(IIC_ADD,offset,data,1);
    return i2c_master_read_register(ICM_I2C_ADDR, offset, 1, data);
}

//int i2c_write(unsigned short offset,unsigned char data)
//{
//    //return i2c_write(IIC_ADD, offset, &data,1);
//    return i2c_master_write_register(ICM_I2C_ADDR, offset, 1, &data);
//}
// i2c_read(IIC_ADD, 0x3B,buf,6);

int i2c_read(unsigned i2c_addr, unsigned short offset, unsigned char * data, unsigned short len)
{
    //return i2c_read(IIC_ADD,offset,data,1);
    return i2c_master_read_register(ICM_I2C_ADDR, offset, len, data);
}

int i2c_read_raw(u8 slave_addr, u8 *data,u16 length)
{
  //return ST_Sensors_I2C_Read((slave_addr>>1), length,data);
  return i2c_master_read_raw(slave_addr, length, data);
}

int i2c_write_raw(u8 slave_addr, u8 *data,u16 length)
{
    //return ST_Sensors_I2C_Write((slave_addr>>1), length,data);
    return i2c_master_write_raw(slave_addr, length, data);
}
