...
enum inv_sensor_type {
	...
	INV_SENSOR_TYPE_KWR                          = 36,  /**< Key word recognition */
...
typedef struct inv_sensor_event
{
	...
	union {
		...
		struct {
			uint8_t      sleep_phase;      /**< the state of sleep phases */
			uint32_t     timestamp;        /**< time stamp of the sleep phase transition (seconds)*/
			int32_t      sleep_onset;      /**< flag to report the sleep */
			int32_t      sleep_latency;    /**< the time delay for the sleep state detection */
			uint32_t     time_in_bed;      /**< the time in bed */
			uint32_t     total_sleep_time; /**< the total sleep time */
			uint8_t      sleep_efficiency; /**< the ratio between total sleep time and time in bed */
		...
		struct {
			int32_t      table[6];         /**< data encrypted table */
		} dataencryption;  
		struct {
		...
		struct {
			uint8_t      status[4];        /**< raw temperature value */
