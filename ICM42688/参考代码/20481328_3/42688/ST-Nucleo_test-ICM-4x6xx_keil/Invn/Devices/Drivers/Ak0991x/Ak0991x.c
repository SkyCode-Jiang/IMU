...
int inv_ak0991x_poll_data(struct inv_ak0991x * s, int16_t * compass_data)
{
	...
	if(s->compass_en) {
		rc = inv_ak0991x_serif_read_reg(&s->serif, AK0991x_REG_ST1, data, 8);
		if(rc != 0)
			return rc;
...
int inv_ak0991x_run_selftest(struct inv_ak0991x * s)
{
	...
	while (counter > 0) {
		result = inv_ak0991x_serif_write_reg(&s->serif, AK0991x_REG_ST1, data, 1);
		if (result)
			goto AKM_fail;
		if ((data[0] & AK0991x_DRDY) == 0)
			counter--;
...
{
	return inv_ak0991x_serif_read_reg(&s->serif, AK0991x_REG_ASAX, &s->compass_sens[0], 3);;
}
