...
int inv_icm406xx_init(struct inv_icm406xx * s, struct inv_icm406xx_serif * serif, void (*sensor_event_cb)(inv_icm406xx_sensor_event_t * event))
{
	int rc;
	...
	if((rc = inv_icm406xx_configure_serial_interface(s)) != 0 )
		return rc;
...
int inv_icm406xx_get_data_from_fifo(struct inv_icm406xx * s)
{
	...
	if((int_status & BIT_INT_STATUS_FIFO_THS) || (int_status & BIT_INT_STATUS_FIFO_FULL)) {
		...
		if((rc = inv_icm406xx_rd_fifo_count(s, data)) != INV_ERROR_SUCCESS)
			return rc;
...
int inv_icm406xx_reset_fifo(struct inv_icm406xx * s)
{
	...
	if ((rc = inv_icm406xx_rd_pwr_mgmt0_gyro_mode(s, &gyr_mode)) != 0 )
		return rc;

	if ((rc = inv_icm406xx_rd_pwr_mgmt0_accel_mode(s, &acc_mode)) != 0 )
		return rc;
	
	/* Workaround HW Bug 9052 only works if at least one sensor is enabled and if accel isn't in low power mode*/
	if ( ( (gyr_mode != ICM406XX_PWR_MGMT_0_GYRO_MODE_OFF) || (acc_mode != ICM406XX_PWR_MGMT_0_ACCEL_MODE_OFF) )
...
static int inv_icm406xx_init_hardware_from_ui(struct inv_icm406xx * s)
{
	...
	*/
	status |= inv_icm406xx_wr_intf_config0_fifo_count_rec(s, ICM406XX_INTF_CONFIG0_FIFO_COUNT_REC_RECORD);
	status |= inv_icm406xx_wr_intf_config0_fifo_count_endian(s, ICM406XX_INTF_CONFIG0_FIFO_COUNT_LITTLE_ENDIAN);
