...
{
	switch(reg) {
	case ICM406XX_ACCEL_CONFIG0_FS_SEL_1g:   return 1000;
	case ICM406XX_ACCEL_CONFIG0_FS_SEL_2g:   return 2000;
	case ICM406XX_ACCEL_CONFIG0_FS_SEL_4g:   return 4000;
	case ICM406XX_ACCEL_CONFIG0_FS_SEL_8g:   return 8000;
	case ICM406XX_ACCEL_CONFIG0_FS_SEL_16g:  return 16000;
	default:          return -1;
	}
}
...
static int save_settings(struct inv_icm406xx * s, struct recover_regs * saved_regs)
{
	...
	result |= inv_icm406xx_read_reg(s, MPUREG_GYRO_ACCEL_CONFIG0, 1, &saved_regs->accel_gyro_config0);
...
static int recover_settings(struct inv_icm406xx * s, const struct recover_regs * saved_regs)
{
	...
	result |= inv_icm406xx_write_reg(s, MPUREG_GYRO_ACCEL_CONFIG0, 1, &saved_regs->accel_gyro_config0);
	/* wait 200ms for gyro output to settle */
	//--yd inv_icm406xx_sleep_us(200*1000);
	delay_us(200*1000);

	return result;
}

static int do_average_on_sensor_output(struct inv_icm406xx * s, int sensor, int self_test_flag, int32_t sensor_result[3])
{
	int result = 0;
	...
	if(self_test_flag) {
		...
		if(sensor == INV_ICM406XX_SENSOR_ON_MASK_GYRO)
			...
			//--yd inv_icm406xx_sleep_us(25*1000);
			delay_us(25*1000);
	}
	...
	if(sensor == INV_ICM406XX_SENSOR_ON_MASK_GYRO) {
		...
		//--yd inv_icm406xx_sleep_us(60*1000);
		delay_us(60*1000);
	}
...
static int check_gyro_self_test(struct inv_icm406xx * s, struct recover_regs * saved_reg)
{
	...
	rc |= do_average_on_sensor_output(s, INV_ICM406XX_SENSOR_ON_MASK_GYRO, (BIT_GYRO_X_ST_EN + BIT_GYRO_Y_ST_EN + BIT_GYRO_Z_ST_EN), ST_ON_gyro);
	...
	for (i = 0; i < 3; i++) {
		...
		/* Check bias */
		if(INV_ABS(s->gyro_st_bias[i]) > (MAX_DEFAULT_ST_GYRO_OFFSET_DPS  * gyro_sensitivity_1dps))
		  result = 0;
	}
...
static int check_accel_self_test(struct inv_icm406xx * s, struct recover_regs * saved_reg)
{
	...
	/* enable self-test for each axis */
	/* then read average gyro digital output for each axis and store them as ST_ON_{x,y,z} in lsb x 1000 */
	rc |= do_average_on_sensor_output(s, INV_ICM406XX_SENSOR_ON_MASK_ACCEL, (BIT_ACCEL_X_ST_EN + BIT_ACCEL_Y_ST_EN + BIT_ACCEL_Z_ST_EN + BIT_ST_REGULATOR_EN), ST_ON_accel);
...
void inv_icm406xx_get_st_bias(struct inv_icm406xx * s, int * st_bias)
{
	...
	for (i = 0; i < 3; i++) {
		...
		st_bias[i + t] = convert_bias(ST_gyro_sensitivity_1dps, s->gyro_st_bias[i], gyro_sensitivity_1dps);
		st_bias[i + t] *= scale;
	}
	...
	for (i = 0; i < 3; i++) {
		/* convert bias to the current FSR */
		st_bias[i + t] = convert_bias(ST_gyro_sensitivity_1dps, s->gyro_lp_st_bias[i], gyro_sensitivity_1dps);
		st_bias[i + t] *= scale;
	}
	...
	for (i = 0; i < 3; i++) {
		/* convert bias to the current FSR */
		st_bias[i + t] = convert_bias(ST_acc_sensitivity_1g, s->accel_st_bias[i], acc_sensitivity_1g);
		st_bias[i + t] *= scale;
	}
	...
	for (i = 0; i < 3; i++) {
		/* convert bias to the current FSR */
		st_bias[i + t] = convert_bias(ST_acc_sensitivity_1g, s->accel_lp_st_bias[i], acc_sensitivity_1g);
		st_bias[i + t] *= scale;
	}
}
...
void inv_icm406xx_set_st_bias(struct inv_icm406xx * s, int * st_bias)
{
	...
	for (i = 0; i < 3; i++) {
		/* convert bias to the current FSR */
		s->gyro_st_bias[i] = convert_bias(gyro_sensitivity_1dps, st_bias[i + t], ST_gyro_sensitivity_1dps);
	...
	for (i = 0; i < 3; i++) {
		/* convert bias to the current FSR */
		s->accel_st_bias[i] = convert_bias(acc_sensitivity_1g, st_bias[i + t], ST_acc_sensitivity_1g);
	...
	for (i = 0; i < 3; i++) {
		/* convert bias to the current FSR */
		s->accel_lp_st_bias[i] = convert_bias(acc_sensitivity_1g, st_bias[i + t], ST_acc_sensitivity_1g);
