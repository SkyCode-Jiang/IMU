...
void PidParameter_init(void)
{
	...
	PID_ROL_Rate.P = 0.835;
  PID_ROL_Rate.I = 0.0095;
	PID_ROL_Rate.D = 0.41;
	
	PID_ROL_Rate.Ilimit_flag = 0; //Roll轴角速度积分的分离标志
	PID_ROL_Rate.Ilimit = 300;    //Roll轴角速度积分范围
	PID_ROL_Rate.Irang = 1000;    //Roll轴角速度积分限幅度（由于电机输出有限，所以积分输出也是有限的）
	
	PID_ROL_Angle.Ilimit_flag = 0;//Roll轴角度积分的分离标志
	PID_ROL_Angle.Ilimit = 35;    //Roll轴角度积分范围			35
	PID_ROL_Angle.Irang = 1000;    //Roll轴角度积分限幅度（由于电机输出有限，所以积分输出也是有限的）		200
  ...
  PID_PIT_Rate.P = 0.765;
	PID_PIT_Rate.I =0.0095;
	PID_PIT_Rate.D = 0.41 ;
	
	PID_PIT_Rate.Ilimit_flag = 0; //Pitch轴角速度积分的分离标志
	PID_PIT_Rate.Ilimit = 300;    //Pitch轴角速度积分范围
	PID_PIT_Rate.Irang = 1000;    //Pitch轴角速度积分限幅度（由于电机输出有限，所以积分输出也是有限的）
